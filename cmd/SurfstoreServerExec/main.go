...
func startServer(hostAddr string, serviceType string, blockStoreAddr string) error {
	...
	// check serviceType for meta, block, or both
	if serviceType == "meta" {
		// (register metaStore server to gRPC server)
		log.Println("meta server")
	...
	if err != nil {
		return fmt.Errorf("[server-main] failed to listen: %v", err)
	}

	if err := grpcServer.Serve(lis); err != nil {
		return fmt.Errorf("[server-main] failed to serve: %v", err)
	}
